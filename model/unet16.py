...
def hab_block(args, n_feats=64, embed_dim=64, num_head=2, mlp_fusion='mlp'):
    mm = []
...
class HAPFSR(nn.Module):
    def __init__(self, args, n_feats=64, number=6, embed_dim=64, num_head=2, mlp_fusion='mlp'):
        ...
        self.SR_branch4 = basic_block(args=args)
        self.SR_up4 = nn.Sequential(*[Upsampler(conv=default_conv, scale=2, n_feats=n_feats)])
        ...
        self.hab_branch4 = hab_block(args)
        ...
        self.Heatmap_branch4 = basic_block(args=args)
        self.Heatmap_up4 = nn.Sequential(*[Upsampler(conv=default_conv, scale=2, n_feats=n_feats)])
    ...
    def forward(self, x):
        ...
        # # # SR step 3 # # #
        for i in range(len(self.SR_branch3) * 2 - 1):
            if i % 2 == 0:
        ...
        for i in range(len(self.SR_branch4) * 2 - 1):
            if i % 2 == 0:
                sr_feature = self.SR_branch4[i // 2](sr_feature)
                heatmap_feature = self.Heatmap_branch4[i // 2](heatmap_feature)
            else:

                sr_feature = self.hab_branch4[i // 2](sr_feature, heatmap_feature)

        sr_feature = self.SR_up4(sr_feature)+ F.interpolate(feature, scale_factor=16, mode='bicubic')
        heatmap_feature = self.Heatmap_up4(heatmap_feature)
